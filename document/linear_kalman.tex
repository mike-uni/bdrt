...
\end{flalign}
Following the derivations of \cite{singpur} we can find closed form expressions for posterior distributions: 
\begin{flalign}
...
\end{flalign}
So let $A_0 = \left(\iwk{0} + F^T\iwk{k}F\right)$ and $b_0^T = \left(\mik{0}^T\iwk{0} + \zik{1}^T\iwk{k}F - Gu_1^T\iwk{k}F\right)^T$ then we integrate over each $z_i$. Notice that $b_0$ contains a $\zik{1}$ term (treated as a constant w.r.t $\zik{0}$) so it needs to be expanded, rearranged and marginalised out in the next step. Focusing on the expansion of $\exp \left[\frac{1}{2} b^T_0A_0b_0 \right]$ and also letting $d_0 = \mik{0}^T\iwk{0}$:
\begin{flalign*}
...
= & \exp \bigg[\frac{1}{2} \left(d_0^TA_0^{-1}d_0 - 2d_0^TA_0^{-1}F^T\iwk{k}Gu_1 + u_1^TG^T\iwk{k}FA_0^{-1}F^T\iwk{k}Gu_1 \right) \bigg] \cdot \\
  & \exp \bigg[\frac{1}{2} \biggl(\zik{1}^T\iwk{k}FA_0^{-1}F^T\iwk{k}\zik{1}  - 2\zik{1}^T\iwk{k}FA_0^{-1}F^T\iwk{k}Gu_1 + 2d_0^TA_0^{-1}F^T\iwk{k}\zik{1} \biggr) \bigg] &&
\end{flalign*}
A brief summary of the calculation so far shows:
\begin{flalign}
\label{eq:if_sumcalc}
&\left(\lprod_{i = 1}^t \frac{1}{|\vk|^{\Ik{i}}} \right) \exp \left[-\frac{1}{2} \biggl(\sum_{i = 1}^T \Ik{i} \yik{i}^T\ivk{k}\yik{i} + u_{i}^TG^T\iwk{k}Gu_{i} + \mik{0}\iwk{0}\mik{0} \biggr)\right] \cdot \\ 
 &(|2\pi A_0|)^{\frac{1}{2}}\exp \bigg[\frac{1}{2}(d_0-F^T\iwk{k}Gu_1)^TA_0^{-1}(d_0-F^T\iwk{k}Gu_1) \bigg] \cdot \\ 
 &\lint \exp \biggl[-\frac{1}{2} \biggl(-\zik{1}^T\iwk{k}FA_0^{-1}F^T\iwk{k}\zik{1}-2d_0^TA_0^{-1}F^T\iwk{k}\zik{1} + \\
 &\qquad 2u_1^TG^T\iwk{k}FA_0^{-1}F^T\iwk{k}\zik{1} + \zik{1}^T\left(\iwk{k} + F^T\iwk{k}F + \Ik{1}H^T\ivk{k}H\right)\zik{1}\; - \\
 &\qquad\qquad 2\left(\Ik{1}\ivk{k}H\yik{1} + \iwk{k}Gu_1 + \zik{2}^T\iwk{k}F - u_2^TG^T\iwk{k}F\right)\zik{1}\; \biggr) \bigg] \cdot \\
  &\qquad\qquad\qquad \bigg[-\frac{1}{2} \left(\; \dots \; \right) \bigg] d\zik{1} \dots d\zik{t} && 
\end{flalign}
Another expansion and rearrangement gives us (inside the integral):
\begin{flalign}
\label{eq:if_compiter}
&\exp \bigg[ -\frac{1}{2} \bigg(\zik{1}^T\left(\iwk{k} + F^T\iwk{k}F + \Ik{1}H^T\ivk{k}H - \iwk{k}FA_0^{-1}F^T\iwk{k}\right)\zik{1} - \\ \nonumber
&\qquad2\bigg(\Ik{1}\ivk{k}H\yik{1} + d_0^TA_0^{-1}F^T\iwk{k} + \iwk{k}Gu_1 - \iwk{k}FA_0^{-1}F^T\iwk{k}Gu_1 + \\ \nonumber
&\qquad\qquad\zik{2}^T\iwk{k}F - u_2^TG^T\iwk{k}F \bigg)\zik{1}\bigg) \bigg]&&
\end{flalign}
Letting:
\begin{flalign}
\label{eq:if_compiter1}
&A_1 = \left(\iwk{k} + F^T\iwk{k}F + \Ik{1}H^T\ivk{k}H - \iwk{k}FA_0^{-1}F^T\iwk{k}\right) \\ \nonumber
&b_1^T = \left(d_1 + \zik{2}^T\iwk{k}F - u_2^TG^T\iwk{k}F \right)^T \\ \nonumber
&d_1 = \left(\Ik{1}\ivk{k}H\yik{1} + d_0^TA_0^{-1}F^T\iwk{k} + \left(\iwk{k} - \iwk{k}FA_0^{-1}F^T\iwk{k}\right)Gu_1\right)&&
\end{flalign}
we can complete the square again, getting from the second part of Equation \eqref{eq:if_compsq}:
\begin{flalign}
\label{eq:if_compiter2}
...
= & \exp \bigg[\frac{1}{2} \left(d_1^TA_1^{-1}d_1 - 2d_1^TA_1^{-1}F^T\iwk{k}Gu_2 + u_2^TG^T\iwk{k}FA_1^{-1}F^T\iwk{k}Gu_2 \right) \bigg] \cdot \\
  & \exp \bigg[\frac{1}{2} \biggl(\zik{1}^T\iwk{k}FA_1^{-1}F^T\iwk{k}\zik{1}  - 2u_2^TG^T\iwk{k}FA_1^{-1}F^T\iwk{k}\zik{1} + 2d_1^TA_1^{-1}F^T\iwk{k}\zik{1} \biggr) \bigg] &&
\end{flalign}
Notice that this has exactly the same form as Equation \eqref{eq:if_compiter}. Iterating the process in Equations \eqref{eq:if_compiter} to \eqref{eq:if_compiter2} for each $t$ in $1 \dots t$
will produce:
\begin{flalign*}
&p(T \giv y^t) = \\ 
&p(T) \lprod^{K_T}_{k = 1} \lint p(\zik{0}) \lprod_{i = 1}^t p(y_i \giv \zik{i}, T)^{\Ik{i}}p(\zik{i} \giv \zik{i-1}, u_t, T) dz^t_k = \\ \nonumber
&p(T)(|2\pi W_0|)^{-\frac{1}{2}}\exp \bigg[-\frac{1}{2} \biggl(\mik{0}\iwk{0}\mik{0} - d_0^TA_i^{-1}d_0 \biggr) \bigg]\lprod^{K_T}_{k = 1}\left(\lprod_{i = 1}^t(|2\pi A_i|)^{\frac{1}{2}}(|2\pi\wk |)^{-\frac{1}{2}}(|2\pi\vk|)^{\frac{-\Ik{i}}{2}}\right)\cdot \\
...
&d_0 = \mik{0}^T\iwk{0} \\
&A_i = \left(\iwk{k} + F^T\iwk{k}F + \Ik{i}H^T\ivk{k}H - \iwk{k}FA_{i-1}^{-1}F^T\iwk{k}\right) \\
&b_i^T = \left(d_i + \zik{i+1}^T\iwk{k}F - u_{i+1}^TG^T\iwk{k}F \right)^T \\
&d_i = \left(\Ik{i}\ivk{k}H\yik{i} + d_{i-1}^TA_{i-1}^{-1}F^T\iwk{k} + \left(\iwk{k} - \iwk{k}FA_{i-1}^{-1}F^T\iwk{k}\right)Gu_i\right)\\
&A_t = \left(\iwk{k} + \Ik{t}H^T\ivk{k}H - \iwk{k}FA_{t-1}^{-1}F^T\iwk{k}\right) \\
&b_t^T = d_t^T \\
&d_t = \left(\Ik{t}\ivk{k}H\yik{t} + d_{t-1}^TA_{t-1}^{-1}F^T\iwk{k} + \left(\iwk{k} - \iwk{k}FA_{t-1}^{-1}F^T\iwk{k}\right)Gu_t\right)&&
\end{flalign*}
