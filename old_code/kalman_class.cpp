...
class Kalman : public Node {
	...
	// Constructors
	Kalman() {};
	Kalman(int const DIMS) : z_vec(DIMS), r_mat(DIMS, DIMS), 
								y_vec(DIMS), s_mat(DIMS, DIMS), 
								kal_g(DIMS, DIMS) {};
	//Declarations
	void zvec(Ref<VectorXd> z_predict, const MatrixXd& FMAT, 
				const VectorXd& zprev);
...
void Kalman::zvec(Ref<VectorXd> z_predict, const MatrixXd& FMAT, 
					const VectorXd& zprev) {
	z_predict = FMAT*zprev;
} 

// A priori covariance estimate matrix
void Kalman::rmat(Ref<MatrixXd> r_predict, const MatrixXd& FMAT, 
					const MatrixXd& WMAT, const MatrixXd& sigmat) {
