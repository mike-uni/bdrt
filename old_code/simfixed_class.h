	...
	for (int i = 0; i < ndset; i++) {
			...
			if (DATA_LENGTH == 0) {
				...
				for (int k = 0; k<matset; k++) {
					...
					predvec = {5,3,7,2,5,9,1,4,6,3,2,1,8};
...
*/					mypred = ("X" + to_string(predvec[0]));
					mysplit = dist.rnorm(predvec[0], 0.5);
					sim.root->pred = mypred;
					sim.root->split = mysplit;
					file4 << sim.root->nnode << " " << mypred << 
							" " << mysplit << endl;
					...
					for (int i = 0; i < num_nodes; i++) {
						int nnum = nodevec[i];
						mypredl = ("X" + to_string(predvec[j]));
						mysplitl = dist.rnorm(predvec[j], 0.5);
						mypredr = ("X" + to_string(predvec[j+1]));
						mysplitr = dist.rnorm(predvec[j+1], 0.5);
					...
					for (int j = 0; j < LOOPS; j++) {
						sim_normx(DATA_LENGTH, path3, 1, 0.5);
...
/*int main() {
	...
	Dist d;
	Simtree simtree;
	simtree.simcfg(cfg, set, d, "new", "testexp.cfg");
}*/
