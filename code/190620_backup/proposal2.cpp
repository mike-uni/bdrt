...
void Prop::mgprop(map<int, Node*>& tree, const vector<int>& leaf_nodes, const double& ALPH, const double& BET, 
		const int& MDIM, const int& NDIM, const int ngrow, const int& npred, const vector<string>& names, 
		map<string, Dataline::type> alltypes, const map<string, string>& allcats, 
		const map<string, vector<double>>& dbounds, const map<string, vector<int>>& ibounds, 
		string sttype, double& lfratio, double& lrratio) {
	...
	sit = subtree.find(rand_leaf);
	growprob = (sit->second->psplit);
	pruneprob = (1-sit->second->psplit);
	if (num_leaves == 1) {
		pgchooseleaf = 1;
		ppchooseleaf = 1;
	}
