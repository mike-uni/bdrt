...
class Move {
	...
	void mgrow(map<int, Node*>& tree, const int node, const int ngrow, const double ALPH, 
		const double BET, const int MDIM, const int NDIM, const vector<string>& names, 
		map<string, Dataline::type> alltypes, const map<string, string>& allcats, 
		const map<string, vector<double>>& dbounds, const map<string, vector<int>>& ibounds); 
...
void Move:: mgrow(map<int, Node*>& tree, const int node, const int ngrow, const double ALPH, 
	const double BET, const int MDIM, const int NDIM, const vector<string>& names, 
	map<string, Dataline::type> alltypes, const map<string, string>& allcats, 
	const map<string, vector<double>>& dbounds, const map<string, vector<int>>& ibounds) {
	//cout << "Inside mgrow: " << endl;

	int nnode = node;
	vector<int> nodevec;
	size_t ctreesize = tree.size();

	//if (ngrow == 1) {
	//grow(tree, node, ALPH, BET, MDIM, names, alltypes, allcats, dbounds, ibounds);
	//}
	//else {
		for(int i = 0; i < ngrow; ++i) {
			nodevec.clear();
			grow(tree, nnode, ALPH, BET, MDIM, NDIM, names, alltypes, allcats, dbounds, ibounds);
			//T.display(tree);
			cout << endl;
			int ntreesize = tree.size();
			if(ctreesize == ntreesize) {
				nnode = nnode;
			} else {
				T.find_leaf_nodes(tree, nodevec, ntreesize);
				nnode = Move::selector(nodevec);
			}
		}
	//}
}
