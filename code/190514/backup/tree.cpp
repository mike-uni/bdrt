...
class Tree {
	public:
		...
		// Constructors
		Tree() {};	
		Tree(const int& nnode, const int& MDIMS) {
			tree[1] = new Node(nnode, MDIMS);
		};
		...
		void find_uleaf(const map<int, Node*>& tree, 
				Dataline::line fline, int& found);
		...
		//map<string, double> get_pred_path(const map<int, Node*>& tree, const int& node);
...
void Tree:: find_prior(const map<int, Node*>& tree) {
	...
	for (it = tree.begin(); it != tree.end(); ++it) {
		if (!it->second->isleaf) {
			prior = prior*(1-it->second->psplit)*(1-it->second->prule);
		}
		else {
