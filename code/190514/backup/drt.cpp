...
int main(int argc, char * argv[]) {
	...
	LLT<MatrixXd> chV(VT);
	MatrixXd LV = chV.matrixL();
	LLT<MatrixXd> chSIG(SIG);
	MatrixXd LSIG = chSIG.matrixL();
	cout << "LSIG " << LSIG << endl;
...
#pragma omp parallel if(dopara == 1) shared(copytrees) firstprivate(leaf_nodes, \
		int_nodes, leaf_nodes_prop, slpostn, WT, VT, HT, FT, GT, fline,\
	       	VTI, WTI, NDIMS, MDIMS, FWIFT, HVIHT, VIHT, WIFT, WIF, ut, \
		yi, UPGWI, GUWI, VTD, WTD, l, k, id, cit, D)	
	{	
#pragma omp for schedule(dynamic, 1) ordered 
	...
	if (l == 0) {
			...
			F.init_tree(F.forest[k]->tree, AT_0, AI_0, ALPH, BET, C_0, D_0, 
				POST_INIT, LPOST_INIT, MU, LSIG, DATALENGTH, D.names, D.alltypes, 
		...
		#pragma omp ordered 
		{
			...
			// KALMAN UPDATE 
			F.kupdate(F.forest[k]->tree, F.forest[k]->found, leaf_nodes, NDIMS, MDIMS, yi, LW, LV, HT.transpose());
