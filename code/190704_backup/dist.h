...
class Dist {
	...
	double dmvnorm(const VectorXd& x, const VectorXd& mu, const MatrixXd& Sig);
...
// Density from multivariate normal
double Dist:: dmvnorm(const VectorXd& x, const VectorXd& mu, const MatrixXd& Sig) {
	...
	return exp(-x.rows()*logsqrt2pi - 0.5*quadform)/L.determinant();
} 
